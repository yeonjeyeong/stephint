    ...
    return (
      <div className="gradient-page flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-medium text-surface-400">학습 기록을 불러오는 중입니다...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="gradient-page min-h-screen">
        <div className="mx-auto max-w-5xl px-5 py-12 md:py-16">
          <div className="mb-10">
            ...
            <h1 className="mb-3 text-3xl font-bold md:text-5xl">내 진단 기록</h1>
            <p className="text-lg text-surface-400">
              이전 결과를 다시 보면서 반복되는 오개념과 힌트 흐름을 복습할 수 있습니다.
            </p>
          </div>
          ...
          {filtered.length === 0 ? (
            <div className="glass-card p-14 text-center">
              ...
              <p className="mb-8 text-surface-500">
                첫 문제를 업로드하면 진단 기록이 이곳에 쌓이기 시작합니다.
              </p>
            ...
            <div className="space-y-4">
              {filtered.map((submission) => {
                ...
                return (
                  <article key={submission.id} className="result-card">
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      ...
                      <Link
                        href={`/student/result/${submission.id}`}
                        className="inline-flex items-center gap-1.5 self-start rounded-full border border-brand-500/20 bg-brand-500/10 px-4 py-2 text-sm font-semibold text-brand-200 transition hover:border-brand-400/35 hover:bg-brand-500/15"
                      >
                        결과 보기
