...
export default function ResultPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [hintMeta, setHintMeta] = useState<{ providerName?: string; fallbackUsed?: boolean } | null>(
    null
  );
  ...
  const requestExtraHint = async () => {
    ...
    try {
      ...
      setExtraHint(
        '지금 목표가 무엇인지 먼저 적고, 그 목표를 위해 이미 맞게 가고 있는 단계가 어디인지 다시 확인해 보세요.'
      );
    ...
    return (
      <div className="gradient-page flex min-h-screen items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-medium text-surface-400">진단 결과를 불러오는 중입니다...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="gradient-page flex min-h-screen items-center justify-center">
        <div className="glass-card max-w-md p-10 text-center">
          ...
          <p className="mb-6 text-sm text-surface-400">
            요청한 진단 결과가 아직 준비되지 않았거나 삭제되었을 수 있습니다.
          </p>
  ...
  const visibleMisconceptionTags = filterVisibleMisconceptionTags(diagnosis.misconceptionTags);
      ...
      <div className="gradient-page min-h-screen">
        <div className="mx-auto max-w-5xl px-5 py-12 md:py-16">
          <Link
            ...
            className="mb-8 inline-flex items-center gap-1.5 text-sm text-surface-500 transition-colors hover:text-brand-400"
          >
            <ArrowLeft size={16} />
            내 기록으로 돌아가기
          </Link>
          ...
          <div className="mb-10">
            ...
            <h1 className="text-3xl font-bold md:text-5xl">나의 진단 결과</h1>
          </div>
          ...
          <div className="mb-6 flex flex-wrap gap-2">
            ...
            {data.promptVersion ? (
              <span className="badge badge-accent text-xs">{data.promptVersion}</span>
            ) : null}
            {data.fallbackUsed ? (
              <span className="badge badge-warm text-xs">fallback 사용</span>
            ) : null}
            {!data.leakageGuardPassed ? (
              <span className="badge badge-warm text-xs">안전 보정 적용</span>
            ) : null}
          </div>
          ...
          <div className="space-y-4">
            ...
            <InfoCard title="오개념 태그" icon={<Tag size={18} />} tone="warm">
              {visibleMisconceptionTags.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {visibleMisconceptionTags.map((tag) => (
                ...
                <p className="text-sm text-surface-400">
                  이번 제출에서는 오개념 태그를 구체적으로 특정하지 못했어요.
                </p>
              )}
            </InfoCard>
                ...
                <div className="space-y-3">
                  ...
                  {hintMeta ? (
                    <div className="flex flex-wrap gap-2 text-xs">
                      {hintMeta.providerName ? (
                        <span className="badge badge-brand text-xs">{hintMeta.providerName}</span>
                      ) : null}
                      {hintMeta.fallbackUsed ? (
                        <span className="badge badge-warm text-xs">fallback 사용</span>
                      ) : null}
                    </div>
                  ) : null}
            ...
            <InfoCard title="다시 도전할 질문" icon={<HelpCircle size={18} />} tone="accent">
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            ...
            <Link href="/student/upload" className="btn-primary flex-1 text-center" id="btn-new-analysis">
              새 문제 분석하기 <ChevronRight size={16} />
            </Link>
          </div>

          <p className="mt-5 text-center text-xs text-surface-600">
            StepHint는 정답을 바로 주지 않고, 스스로 다음 단계를 생각하도록 돕습니다.
          </p>
        </div>
      </div>
    </RoleGuard>
